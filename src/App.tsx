...
import { Crown, Clock3, History, RefreshCcw, Timer, Wand2 } from "lucide-react"
...
function getRoundInfo(kyokuIndex: number, honba: number): {
  wind: RoundWind
  number: number
  label: string
} {
...
function App() {
  ...
  const [editNames, setEditNames] = useState<string[]>(() => createDefaultNames())
  ...
  const playerRanks = useMemo(() => computePlayerRanks(state.points), [state.points])
  ...
  const hasLastSettlement = Boolean(
    state.lastSettlementBefore && state.lastSettlementAfter && state.lastSettlementMeta,
  )
  ...
  function buildHistoryEntry(
    ...
    const dealerLabel = state.names[state.dealerIndex] ?? PLAYER_LABELS[state.dealerIndex]
    return {
  ...
  function handleEditRoundConfirm() {
    const kyokuIndex =
      editWind === "东" ? editNumber - 1 : 3 + editNumber
    setState((prev) => ({
      ...prev,
  ...
  function handleTsumoConfirm(): boolean {
    ...
    setState((prev) => {
      ...
      const dealerName = prev.names[prev.dealerIndex] ?? PLAYER_LABELS[prev.dealerIndex]
      const basePoints = calcBasePoints(han, fu)
      const deltas = [0, 0, 0, 0]
      const riichiIndices: SeatIndex[] = []
      ...
      const riichiPlayers = riichiIndices.map((i) => prev.names[i] ?? PLAYER_LABELS[i])
      ...
      const entry = buildHistoryEntry("tsumo", description, riichiCount, riichiPlayers, deltas)
  ...
  function handleRonConfirm(): boolean {
    ...
    setState((prev) => {
      ...
      const entry = buildHistoryEntry("ron", description, riichiCount, riichiPlayers, deltas)
  ...
  function handleDrawConfirm(): boolean {
    ...
    setState((prev) => {
      ...
      const tenpaiNames = prev.names
        .filter((_, idx) => drawTenpai[idx])
        .join("、") || "无"
      const notenNames = prev.names
        .filter((_, idx) => !drawTenpai[idx])
        .join("、") || "无"
      ...
      const entry = buildHistoryEntry("draw", description, riichiCount, riichiPlayers, deltas)
                ...
                <div className="max-h-80 space-y-2 overflow-y-auto pr-2 text-xs">
                  <Table>
                    ...
                    <TableBody>
                      {state.history.map((h) => {
                        const namesSnapshot =
                          Array.isArray(h.playerNames) && h.playerNames.length === 4
                            ? h.playerNames
                            : state.names
                        ...
                        } else if (typeof h.riichiCount === "number" && h.riichiCount > 0) {
                          riichiLabel = `${h.riichiCount} 人`
                        }
                        const deltas = Array.isArray(h.deltas) ? h.deltas : undefined
                  ...
                  <Dialog open={drawOpen} onOpenChange={setDrawOpen}>
                    ...
                    <DialogContent className="max-w-md">
                      <DialogHeader>
                        ...
                        <DialogDescription>
                          选择听牌家与当局立直情况，系统会自动按照 3,000 点规则结算并累加场供。
                        </DialogDescription>
                      </DialogHeader>
                      <div className="mt-2 space-y-4 text-xs">
                        <div>
                      ...
                      <div className="mt-3 rounded-lg bg-slate-50/80 px-3 py-2">
                        ...
                        <div className="mt-2 text-[11px] text-slate-500">
                          场供：
                          {formatPoints(drawPreview.kyotakuBefore * 1000)} →{" "}
                          {formatPoints(drawPreview.kyotakuAfter * 1000)} 点
                      ...
                      <div className="mt-2 space-y-4 text-xs">
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <Select
                              value={tsumoWinner !== null ? String(tsumoWinner) : ""}
                              onValueChange={(v) =>
                      ...
                      <div className="mt-3 rounded-lg bg-slate-50/80 px-3 py-2">
                        ...
                        {tsumoPreview ? (
                          <div className="space-y-3">
                            ...
                            {tsumoPreview.winnerIndex !== null && (
                              <div className="mt-2 text-[11px] text-slate-600">
                                ...
                                <div className="mt-1 text-[11px] text-slate-500">
                                  （含本场加成、场供{" "}
                                  {formatPoints(tsumoPreview.kyotakuIncome)} 点、
                                  立直棒收入{" "}
                            ...
                            <div className="mt-2 text-[11px] text-slate-500">
                              场供：
                              {formatPoints(tsumoPreview.kyotakuBefore * 1000)} →{" "}
                              {formatPoints(tsumoPreview.kyotakuAfter * 1000)} 点
                            </div>
                          </div>
                        ) : (
                      ...
                      <div className="mt-2 space-y-4 text-xs">
                        <div className="grid grid-cols-2 gap-3">
                          <div>
                            ...
                            <Select
                              value={ronWinner !== null ? String(ronWinner) : ""}
                              onValueChange={(v) =>
                          ...
                          <div>
                            ...
                            <Select
                              value={ronLoser !== null ? String(ronLoser) : ""}
                              onValueChange={(v) =>
                          ...
                          <div className="space-y-3">
                            ...
                            {ronPreview.winnerIndex !== null && (
                              <div className="mt-2 text-[11px] text-slate-600">
                                ...
                                <div className="mt-1 text-[11px] text-slate-500">
                                  （含本场加成、场供{" "}
                                  {formatPoints(ronPreview.kyotakuIncome)} 点、
                                  立直棒收入{" "}
                            ...
                            <div className="mt-2 text-[11px] text-slate-500">
                              场供：
                              {formatPoints(ronPreview.kyotakuBefore * 1000)} →{" "}
                              {formatPoints(ronPreview.kyotakuAfter * 1000)} 点
                            </div>
                          </div>
                        ) : (
                          <div className="text-[11px] text-slate-400">
