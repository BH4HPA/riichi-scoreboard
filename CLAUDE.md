...
- `apps/web` — Vite + React 19 + Tailwind v4 + radix primitives. Routes: `/` landing (device routing:
  ...
  `features/hand/TileFace` (fixed pixel sizes, baseline-aligned with sideways meld tiles) and `features/hand/TileKey` (fills its grid cell — the tile _is_ the hit area; used by `TileGrid` for the tile keyboard, the replace sheet and the ten all-tiles board).
...
- 《天》 two-player rooms (`packages/core/src/ten/`, `apps/web/src/features/ten/`): non-zero-sum (each side
  ...
  nothing; only the seat's owner or a local seat, like `setReady`), `tenMark {tile, on, entries}` (B only: strike / restore one tile on the all-tiles board — explicit `on`, so idempotent; stale-tolerant because taps come faster than broadcasts; replaces `present` without touching the undo stack, like `declareRiichi`, so 34 taps cannot push settlements out of it and tapping again is the undo. The board is only the defender's elimination aid: turns, tiles per turn and whether a guess hit all happen verbally at the table and are not modelled), `tenDraw {reason:
noDeclare | guessed | exhausted}` (honba +1, dealer stays), `tenTsumo {value}` (B only; the winner is always
  ...
  value must be tsumo and its riichi flag must equal the declaration). Declarations are ordinary undoable steps (no cancel command), so Stage B can occur more than once in a round: the draft stamp (`tenDraftStamp`) carries the declaration's identity (attacker + riichi) but not the board marks, and every dialog that records a result closes itself when the stamp changes under it — tsumo / ron forms through `useDraft`, everything else (yonma draw / abortive / chombo / adjust, ten draw confirmations) through `drafts/useCloseOnStale`; without it two phones holding the same confirmation open would record the round twice; `endGame` works in either stage, keeps `stage` in the snapshot (undo
  ...
  and `hand` (`features/rules/fields.ts` `ruleGroupsFor(kind)`: a ten room shows only those two groups minus the `yonmaOnly` fields — pao, noten bappu, nagashi mangan, renhou, kokushi ankan chankan: no ron, no point transfer, no noten penalty — so every control shown really changes scoring); results are not written to `game_results` (personal stats
  ...
  also plays music via `RiichiMusicRow`, 「听牌宣言」), `GuessBoard` (all 34 tiles through the shared `TileGrid`; titled 「排除记录」 in the UI; tap to strike, tap again to restore, optimistic — the override is dropped when the ack arrives, which the server sends after the state broadcast; replaces the history column on the wide console during Stage B, when the shell shows its 「记录」 button instead (`asideIsHistory`), and sits first on the narrow console; who may tap is `stageB/canPick.ts`, the same rule as declaring: a device player's own phone only, anyone for a local player),
  `TenTsumoDialog` (same `ValuePicker` / camera chain as yonma with `riichiLock` from the declaration;
  recognitions keep `source: "room"`), `TenStageHint` (what to do in this stage, always on the TV), time-up copy by stage (`clock/marks.ts`: Stage B "finish this round", Stage A "please end" + the 终局 button turns accent),
  `TenGuide` (rule explainer from core `ten/guide.ts`: on phones all five sections are stacked and an IntersectionObserver reports the one being read, so a cast TV follows the reader; the TV shows one section under a tab strip; called 「玩法说明」 in the UI; reaching the bottom of the scroll selects the last section; castable from the phone lobby and the in-game 规则 sheet via the `tenGuide` intent (the read-only rule parameters below it in that sheet are not cast); the console opens it locally from the lobby and from 操作 during a game — the console lobby mounts a mirror layer only for it), settlement mirror via
  the `tenSettlement` intent.
- Room phases: `lobby` → `playing` → `finished` (→ `lobby` via `toLobby`; `newGame` = abandon the current game and start over, allowed while `playing` as well as `finished`, never undoable, and an abandoned game writes no results), plus `closed` after `dissolve`
