...
/**
 * 手机对局页的壳：主体一列 + 底部工具栏（记录 / 番符表 / 规则 / 我的）与它们的弹层。
 * 与对局模型无关——主体、历史与规则页的内容由各房型的对局页填进来；
 * 番符表与规则页的「投到电视」开关在这里管（关掉弹层即复位）。
 */
export function PhoneGameShell({
  ...
  history: ReactNode;
  historyCount: number;
  /** 规则页：标题下的说明、内容，以及打开「投到电视」时发给电视的意图 */
  rulesSheet: { description: string; content: ReactNode; intent: UiIntent };
}) {
  ...
  useMirror(cast && sheet === "rules", rulesSheet.intent, true);
  ...
  return (
    <div className="mx-auto flex min-h-dvh max-w-md flex-col gap-3 px-4 pb-28 pt-4">
      ...
      <Dialog open={sheet === "history"} onOpenChange={closeSheet}>
        <DialogContent title="历史记录" description={`共 ${historyCount} 条`}>
          {history}
        </DialogContent>
      </Dialog>
      <Dialog open={sheet === "reference"} onOpenChange={closeSheet}>
        <DialogContent title="番符表" className="h-[92dvh] sm:max-w-2xl">
          ...
          <ReferenceSheet rules={rules} view={refView} onViewChange={setRefView} />
        </DialogContent>
      </Dialog>
      <Dialog open={sheet === "rules"} onOpenChange={closeSheet}>
        <DialogContent title="房间规则" description={rulesSheet.description}>
          <CastSwitch checked={cast} onCheckedChange={setCast} />
          {rulesSheet.content}
        </DialogContent>
      </Dialog>
