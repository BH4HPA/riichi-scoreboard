import type { RecognitionCreated, RecognitionPatch } from "@riichi/core";
import { api } from "@/api/client";

/** 上传裁剪后的照片并建记录（照片留作训练数据）。 */
export function createRecognition(blob: Blob, token: string): Promise<RecognitionCreated> {
  return api<RecognitionCreated>("/api/recognitions", { method: "POST", raw: blob, token });
}

export function patchRecognition(
  id: string,
  patch: RecognitionPatch,
  token: string,
): Promise<void> {
  return api<void>(`/api/recognitions/${id}`, { method: "PATCH", body: patch, token });
}
