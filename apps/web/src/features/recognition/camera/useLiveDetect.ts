...
interface Options {
  ...
  /** 界面转了多少度（帧按它转正后再识别），以及这个方向是不是有人定过 */
  view: { rotation: Rotation; known: boolean };
  active: boolean;
 ...
 * 用户看到的框会慢慢落后于画面。
 */
export function useLiveDetect({ detector, videoRef, view, active, onFrame }: Options): void {
    ...
    const step = () => {
      ...
      void createImageBitmap(video)
        .then((bitmap) => {
          ...
          detector.infer(bitmap, viewRef.current);
        })
        .catch(() => {
          inFlight = false;
        });
