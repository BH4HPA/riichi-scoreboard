...
拍照识别（建设中）：牌面检测模型的训练、导出与发布在 `ml/`（见 `ml/README.md`）；类目录与当前发布的模型记录在 `packages/core/src/recognition/manifest.json`，模型对象放 `https://static.bitego.net/riichi/models/<id>.onnx`（`ci/upload-model.sh` 上传后把打印的片段填进 manifest 的 `model`，为 `null` 时不显示识别入口）。
