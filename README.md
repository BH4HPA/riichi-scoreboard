...
拍照识别（建设中，目前只有模型侧）：牌面检测模型的训练、导出与发布在 `ml/`（见 `ml/README.md`）；类目录与当前发布的模型记录在 `packages/core/src/recognition/manifest.json`，模型对象放 `https://static.bitego.net/riichi/models/<id>.onnx`（`ci/upload-model.sh` 校验、上传并把 `model` 写回 manifest；为 `null` 表示尚未发布）。应用侧接入是下一阶段。
