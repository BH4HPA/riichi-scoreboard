...
<p align="center">
  🌐 <a href="https://riichi.ruivon.cn"><strong>riichi.ruivon.cn</strong></a> · 电脑或电视打开选房型开主控台，手机打开就是加入页
</p>
...
- **拍照识别手牌**：不用框选、横竖屏都行：对准手牌，它自己在画面里找到手牌（牌河留在上方不碍事），认稳了就自动定格，牌面自动填好并算番。模型是自训的 YOLO11n，在手机浏览器里用 onnxruntime-web 推理，iPhone 15 上一帧不到 300 ms。
...
1. 电视、电脑或平板打开 [riichi.ruivon.cn](https://riichi.ruivon.cn)，选「创建四人麻将房间」或「创建二人麻将房间」，房间建好后显示二维码（上次的房间还开着时，按钮会写「继续 … 房间码」，旁边可以「新建」）。
...
出自福本伸行《天 天和通りの快男児》：两个人打，没有点棒往来，各自累计得分，限时 1 小时，得分高者胜。每一局分两个阶段——

- **Stage A**：照常摸打（可吃碰杠），先听牌的一方在手机上按「▶ 立直」（用掉 1 根立直棒，每人 10 根）或「听牌宣言」，成为进攻方；18 巡无人宣言则流局。
- **Stage B**：防守方每轮指定 2 张牌，猜中进攻方任意一张待牌即流局；没猜中，进攻方连摸 5 张，摸到就是自摸和，否则再指定，直到王牌。电视右侧这时变成全牌型板，防守方在手机上点一下划掉一张、再点恢复，用来排除；猜牌本身在牌桌上口头进行。

和牌只有自摸一种，得分等于这手牌在四人麻将里自摸的总收入（含本场）；手牌同样可以拍照识别。大厅里的「玩法说明」可以投到电视，对局中电视也常驻当前阶段该做什么。计时不公开，只在剩 10 分钟、5 分钟和时间到时各提示一次，由玩家点「终局」。二人麻将的对局不计入个人战绩。
...
- 指示牌紧贴手牌，牌河离远一点——挨得太近的牌河末行会被当成指示牌。

一直认不稳时，取景画面上会写出原因（比如多认了几张），也可以随时按右下角快门手动定格；横着拍点一下快门旁的旋转按钮（授权陀螺仪后会自动跟着转）。每次识别的定格照片和结算确认后的手牌会上传保存，用作模型的训练数据；每人每小时最多识别 60 次。首页的「拍照算点数」不进房间也能用：设好场风、自风、本场和规则，拍一手牌、核对识别结果，点「识别正确」就给出番符、役种和点数；确认的手牌同样用作训练数据。
