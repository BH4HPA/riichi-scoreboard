...
每次识别都留存了定格照片（定格那一帧收紧到被采信的牌、留 0.35 张牌的边，牌河不进照片）、照片坐标系里的检测框与用户最终确认的手牌（`recognitions` 表，`source` 区分房间结算 `room`、已下线的开发者标注页 `label` 与拍照算点数页 `calc`）。
...
2026-09 取景改成自动范围以后要知道的两件事：

- 布局规则改过（抗牌河、表里张数规则）。规则上线前拍的、以及手机上还缓存着旧前端时拍的记录，`recognized`
  是旧规则算的，导出时用新规则重跑对不上，会以「布局结果与当初记录的不一致」进人工队列——只是降级，不会错标；
  `model_id` 相同，从记录里分不出新旧。
- 定格照片收紧到被采信的牌、四周留 0.35 张牌的边。紧贴着指示牌的牌河行可能在照片边缘露出不到三成的残片，
  它们的框中心在照片外、不进记录，所以不会触发「有框没被采信」。量很小；若发现 auto 队列里这类照片多，导入时按
  「块边缘有被裁掉的原始框」降级即可。
...
- 整手牌完全正确率：`bench_dets.py` + `recognize-bench.ts` 在 `data/bench/` 上的精确匹配比例（暗牌 + 和张 + 副露 + 指示牌全对），是上线门槛（目标 ≥ 90%）。v1 + 当前布局规则：32/33（唯一失败是竖着拍、带牌河的那张），v0：18/33。
