...
每次识别都留存了定格帧、检测框与用户最终确认的手牌（`recognitions` 表，`source` 区分房间结算 `room` 与拍照算点数页 `label`——值沿用改名前的标注页，是数据契约）。
判据是 `corrected IS NOT NULL`：房间来的记录只有结算命令被接受之后才回填，而牌桌上另外三个人不会允许
错误的牌局录进系统——「触发了结算」本身就是一次人力校验，**不看用户改没改过**。`label` 来的记录在玩家点「识别正确」时回填，没有牌桌上另外三个人把关，但玩家要拿它算点数，同样算一次校验。
