#!/usr/bin/env bash
# 训练 YOLO11n。用法：scripts/train.sh [name] [extra yolo args...]
...
# Apple Silicon 用 device=mps；个别算子回退 CPU 需要 PYTORCH_ENABLE_MPS_FALLBACK=1。
set -euo pipefail
...
uv run yolo detect train \
  ...
  imgsz=640 epochs=100 batch=32 device=mps \
  fliplr=0 \
